...
public class ReportDownloadUtil {
	...
	public static void generateSummaryReports(String executionId) throws Exception {
		...
		JsonObject executions = retrieveTestExecutions(OFFLINE_TOKEN, executionId);
		...
		downloadExecutionSummaryReport(deviceId, executionId, OFFLINE_TOKEN);
		// }
	...
	public static void generateTestReport(String executionId) throws Exception {
		...
		while(!executions.get("metadata").getAsJsonObject().get("processingStatus").getAsString().equalsIgnoreCase("PROCESSING_COMPLETE"))
		{
			executions = retrieveTestExecutions(OFFLINE_TOKEN, executionId);
		}
		...
		for (int i = 0; i < executions.getAsJsonArray("resources").size(); i++) {
			...
			testExecution = executions.getAsJsonArray("resources").get(i).getAsJsonObject();
			
			System.out.println("generateTestReport I got testExecution");
			
			String testId = testExecution.get("id").getAsString();
			
			System.out.println("generateTestReport I got testID" + testId);
			
			String testName = testExecution.get("name").getAsString();
			
			System.out.println("generateTestReport I got testName" + testName);
			JsonObject platforms = testExecution.getAsJsonArray("platforms").get(0).getAsJsonObject();
			
			System.out.println("generateTestReport I got platforms" + platforms);
			
			String deviceName = platforms.get("deviceId").getAsString();
			
			System.out.println("generateTestReport I got deviceName" + deviceName);
			
			System.out.println("generateTest - getting ready downloadTestReport - " + testId + " " + deviceName + " " + testName + " " + OFFLINE_TOKEN);

			Thread.sleep(4000);
			
			downloadTestReport(testId, deviceName + "_" + testName.replace(" ", "_"), OFFLINE_TOKEN);
		}
	...
	private static void downloadTestReport(String testId, String fileName, String accessToken)
		...
		System.out.println("getting ready downloadFileAuthenticated " + fileName + " " + uriBuilder.toString() + " " + accessToken);
	...
	private static void downloadFileToFS(HttpGet httpGet, String fileName, String suffix, String description)
		...
		System.out.println("i'm in downloadFileToFS " + fileName + " " + suffix + " " + description + " number " + i++);
		...
		if (ConfigurationManager.getBundle().getString("proxyHost") != null
			...
			HttpClient httpClient = HttpClientBuilder.create().build();
			System.out.println("i'm in downloadFileToFS running execute");
			
			response = httpClient.execute(httpGet);
			System.out.println("i'm in downloadFileToFS finished execute");
			System.out.println("i'm in downloadFileToFS response output " + response.toString());
			System.out.println("i'm in downloadFileToFS response code " +response.getStatusLine().getStatusCode());
			System.out.println("i'm in downloadFileToFS response content " +response.getEntity().getContent());
		}
