...
public class QuantumReportiumListener extends ReportiumTestNgListener implements QAFTestStepListener, ITestListener {
    ...
    protected ReportiumClient createReportiumClient(ITestResult testResult) {
        ...
        String projCommit = null;
        Properties prop = new Properties();
        InputStream input = null;
        String newCommitID = null;
        String newCMTID = null;
        try {
            input = new FileInputStream("resources/new.properties");
            // load a properties file
            prop.load(input);
            // get the property value and print it out
            newCommitID = prop.getProperty("projectCommitID");
            newCMTID = prop.getProperty("cmtID");

        } catch (IOException ex) {
            //ex.printStackTrace();
            System.out.println("No alternate properties file found.");
        } finally {
            if (input != null) {
                try {
                    input.close();
        ...
        projCommit = newCommitID;
        if((projCommit == null) || (projCommit.equals("null"))) {
            allTags = xmlTestName + "," + suiteName + "," + (System.getProperty("reportium-tags") == null ? "" : "," + System.getProperty("reportium-tags"));
        }
        else {
            allTags = xmlTestName + "," + suiteName + "," + projCommit + ","+ newCMTID + ","+ (System.getProperty("reportium-tags") == null ? "" : "," + System.getProperty("reportium-tags"));
            }
        Object testInstance = testResult.getInstance();
